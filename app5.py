...
# ✅ NEW: Let the user optionally attach a photo for the recipe they see
uploaded_photo = st.file_uploader(
    "Optional: upload a photo for the current recipe (PNG/JPG)", 
    type=["png", "jpg", "jpeg"]
)
...
if st.button("🥗 Show Today's Recipe"):
    ...
    # ✅ NEW: If a photo was uploaded, show it beneath the recipe box
    if uploaded_photo is not None:
        st.image(uploaded_photo, caption=f"{recipe['name']} (your photo)", use_column_width=True)
    else:
        st.caption("Tip: upload a PNG/JPG above to display a photo for this recipe.")
else:
